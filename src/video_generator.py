"""Build YouTube Shorts MP4 using MoviePy 2.x."""
...
def _load_bg_image(path: Path | None, idx: int) -> np.ndarray:
    if path and path.exists():
        try:
            ...
            left = (img.width - W) // 2
            top = (img.height - H) // 2
            return _pil_to_np(img.crop((left, top, left + W, top + H)))
...
def _render_frame(bg: np.ndarray, title: str, subtitle: str, progress: float, watermark: str) -> np.ndarray:
    ...
    for line in textwrap.fill(title, width=18).split("\n"):
        ...
        x = (W - lw) // 2
        draw.text((x + 3, 120 + 3), line, font=title_font, fill=(0, 0, 0, 180))
        draw.text((x, 120), line, font=title_font, fill=(255, 255, 255, 255))

    # Subtitle pill
    if subtitle:
        sub_font = _try_font(56)
        lines = textwrap.fill(subtitle, width=24).split("\n")
        y_sub = H - 250
        for line in lines:
            bbox = draw.textbbox((0, 0), line, font=sub_font)
            lw, lh = bbox[2] - bbox[0], bbox[3] - bbox[1]
            x = (W - lw) // 2
            draw.rounded_rectangle([(x - 16, y_sub - 8), (x + lw + 16, y_sub + lh + 8)],
    ...
    # Progress bar
    bar_y = H - 90
    draw.rectangle([(0, bar_y), (W, bar_y + 8)], fill=(50, 50, 50, 200))
    draw.rectangle([(0, bar_y), (int(W * progress), bar_y + 8)], fill=(255, 50, 50, 255))

    # Watermark
    wm_font = _try_font(38)
    draw.text((W - 24, H - 60), watermark, font=wm_font, fill=(200, 200, 200, 180), anchor="rm")
...
class VideoGenerator:
    def generate(
        ...
        log.info("Building video slug=%s | %d scenes", slug, len(image_paths))
        ...
        for scene_idx, img_path in enumerate(image_paths):
            ...
            sub_text = " ".join(seg["text"] for seg in active_subs)

            def make_frame(t, _bg=bg_np, _sub=sub_text,
                           _start=scene_start, _total=real_duration):
                progress = min((_start + t) / max(_total, 1), 1.0)
                return _render_frame(_bg, title, _sub, progress, config.CHANNEL_WATERMARK)

            clip = VideoClip(make_frame, duration=scene_duration)
            clip = clip.with_fps(FPS)

            # Fade transitions — MoviePy 2.x uses with_effects + vfx
            try:
                from moviepy import vfx
                if scene_idx > 0:
                    clip = clip.with_effects([vfx.FadeIn(0.4)])
                if scene_idx < n_scenes - 1:
                    clip = clip.with_effects([vfx.FadeOut(0.4)])
            except Exception:
                pass  # Skip fades if vfx not available

            clips.append(clip)

        final_video = concatenate_videoclips(clips, method="compose")
        final_video = final_video.with_audio(audio_clip.subclipped(0, final_video.duration))
